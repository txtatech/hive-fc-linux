...
// New function to get .mp3 files from the blobs
function getMp3Files() {
  const mp3Files = [];
  blobs.forEach((directoryBlobs, directory) => {
    directoryBlobs.forEach((blob, key) => {
      if (blob.type === 'video/mp3') {
        mp3Files.push({ name: key, url: URL.createObjectURL(blob) });
      }
    });
  });
  return mp3Files;
...
function autoPlayVideos() {
    ...
    function playNextVideo() {
        if (currentVideoIndex < mp3Files.length) {
            loadVideo(mp3Files[currentVideoIndex].url);
            currentVideoIndex++;
        } else {
...
export { getBlobFromURLOrFilename, createObjectURLInDirectory, handleScriptResourceRequest, getMp3Files, autoPlayVideos };
